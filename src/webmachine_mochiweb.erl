...
loop(MochiReq, Name) ->
    case new_webmachine_req(MochiReq) of
      ...
      Req ->
        ...
        PriorityDispatchList = webmachine_router:get_priority_routes(Name),

        try
            PriorityPath =
                webmachine_dispatcher:dispatch(
                    Host,
                    Path,
                    PriorityDispatchList,
                    RD
                ),
            PathMatch =
                case PriorityPath of
                    ...
                    PriorityMatch when is_tuple(PriorityMatch) ->
                        PriorityMatch
                end,
            case PathMatch of
                ...
                {Mod, ModOpts, HostTokens, Port, PathTokens, Bindings, AppRoot, StringPath} ->
                    {ok, XReq1} =
                        webmachine_request:load_dispatch_data(
                            Bindings,
                            HostTokens,
                            Port,
                            PathTokens,
                ...
                {ok, Resource} = 
                    webmachine_resource:wrap(Mod, ModOpts),
                {ok, RS2} = 
